 ...
 * 
 * Reusable modal confirmation component for critical device operations.
 * Displays device summary and requires explicit typed confirmation to proceed.
 * 
 * Security Features:
...
export interface ConfirmActionModalProps {
  ...
  /** Custom action text for the operation */
  actionText?: string;
 ...
 * Format: "[Model] CONFIRM" as per requirements
 */
function generateConfirmationPhrase(device: DeviceInfo): string {
 ...
 */
function getOperationWarning(operationType: string, actionText?: string, pluginName?: string): string {
  if (actionText) {
    return actionText;
  }
...
export function ConfirmActionModal({
  ...
  const confirmationPhrase = generateConfirmationPhrase(device);
  const warningMessage = getOperationWarning(operationType, actionText, pluginName);
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="bg-grimoire-obsidian-light border border-grimoire-electric-blue/30 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm font-tech">
              ...
              <div>
                <span className="text-dark-muted">Device Type:</span>
                <p className="text-grimoire-neon-cyan font-semibold">
                  {device.deviceType || 'Unknown'}
                </p>
              </div>
              {(device.properties?.battery_level || device.properties?.battery) && (
                <div>
          ...
          {/* Confirmation Input - Requires Typing Phrase */}
