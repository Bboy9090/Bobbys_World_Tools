    ...
    try {
      const response = await fetch('/api/v1/trapdoor/unlock', {
        ...
        body: JSON.stringify({
          ...
          confirmation: {
            serial: serialConfirmation,
            unlock: unlockConfirmation,
          },
        }),
      });
      ...
      if (!envelope.ok) {
        throw new Error(envelope.error?.message || 'Unlock operation failed');
      }
      ...
      setOperationId(envelope.data?.operationId || envelope.meta?.requestId || null);
